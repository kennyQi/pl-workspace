...
CREATE TABLE `cal_flow` (
  ...
  `rule` varchar(50) DEFAULT NULL COMMENT '规则编号',
  ...
  `is_cal` tinyint(4) DEFAULT '1' COMMENT '是否是计算：1-是，0-否',
  PRIMARY KEY (`cal_id`)
...
CREATE TABLE `jf_account` (
  ...
  `amount_todo` decimal(10,0) DEFAULT '0' COMMENT '在途、未结算',
  PRIMARY KEY (`id`)
...
CREATE TABLE `jf_flow` (
  ...
  `ref_flow_id` varchar(50) DEFAULT NULL COMMENT '依据的流水ID',
