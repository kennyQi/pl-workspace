 ...
 * 
 */
public class HjfSignLogic extends BaseSingleRule implements RuleLogic {
	...
	public CalResult compute(Serializable flow) throws Exception {
		...
		LogFactory.getLog(HjfSignLogic.class).info("经典复古卷发");
		// 判断会员等级
		if (trade.LEVEL_SILVER.trim().equals(level.trim())) {
			LogFactory.getLog(HjfSignLogic.class).info("白银会员");
			...
			LogFactory.getLog(HjfSignLogic.class).info("钻石会员");
			str += "钻石会员，";
		} else {
			integralIncrement = Integer.parseInt(rule.parameters.get("norjf"));// @REMARK@普通用户签到积分
			LogFactory.getLog(HjfSignLogic.class).info("普通会员");
			str += "普通会员，";
		}
		...
		if (countDays < 8) {// 用户连续签到没有超过七天
			integral += Integer.parseInt(new java.text.DecimalFormat("0").format(integralIncrement * incrementRatio));
		} else if (countDays >= 8 && countDays < 16) {// 用户连续签到超过七天但小于16天
			incrementRatio = Double.parseDouble(rule.parameters.get("jfRatio1"));// @REMARK@签到连续七天之后积分倍数
			integral += Integer.parseInt(new java.text.DecimalFormat("0").format(integralIncrement * incrementRatio));
		} else if (countDays >= 16) {// 用户签到超过16天
			incrementRatio = Double.parseDouble(rule.parameters.get("jfRatio2"));// @REMARK@签到连续15天之后积分倍数
			integral += Integer.parseInt(new java.text.DecimalFormat("0").format(integralIncrement * incrementRatio));
		}
