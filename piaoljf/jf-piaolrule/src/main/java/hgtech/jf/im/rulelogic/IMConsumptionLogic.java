 ...
 */
package hgtech.jf.im.rulelogic;
...
/**
 * @类功能说明：汇金宝im下单消费规则，各店铺自定义自己的积分比例
 * @author xinglj
 *
 */
public class IMConsumptionLogic extends BaseSingleRule implements RuleLogic {
	...
	public CalResult compute(Serializable flow) throws Exception {
		...
		String merchant = rule.parameters.get("merchant");//@REMARK@交易所在店铺代码
		if (!trade.TRADE_CONSUMPTION.trim().equals(trade.tradeName.trim())) {
			return null;
		}else if(!merchant.equals(trade.merchant))
			return null;
