...
public class PiaolDealLogic  extends BaseSingleRule implements RuleLogic{
	...
	public CalResult compute(Serializable flow) throws Exception {
		...
		if(!"deal".equals(trade.tradeName)){
			return null;
		}
		...
		int totleDays=Integer.parseInt(rule.parameters.get("totleDays"));//@REMARK@用户需要在一个自然月内连续多少天之后才会的到成长值
		int countDays1=0;//记录上个月内该用户的消费天数
		int countDays2=0;//记录本月内该用户的消费天数
		List<Date> shoppingDate=null;//存储最近两个月内的所有购物日期
		Integer integralIncrement=Integer.parseInt(rule.parameters.get("IntegralIncrement"));//@REMARK@在一个自然月内会员规定的天数内都有购物增加的积分数量
		...
		if(session==null){//无当前用户
			...
			countDays2=1;
			ca.setTime(dealDate);
			str+=ca.get(ca.YEAR)+"年"+(ca.get(ca.MONTH)+1)+"月,消费天数："+countDays2;
		}else{//用户信息存在
