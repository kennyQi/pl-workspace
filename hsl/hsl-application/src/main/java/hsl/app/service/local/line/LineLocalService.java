...
import hsl.app.dao.line.DayRouteDAO;
import hsl.app.dao.line.LineDAO;
import hsl.app.dao.line.LineImageDAO;
import hsl.app.dao.line.LineSnapshotDAO;
...
public class LineLocalService extends BaseServiceImpl<Line, HslLineQO, LineDAO>{
	...
	private void getAndSetLineData(InitLineCommand command) throws Exception {
		...
		HgLogger.getInstance().info("renfeng", LogFormat.log("直销admin同步线路数据，分销返回结果集：",JSON.toJSONString(response) ));
		try{
	...
	public void SaveOrUpdateLine(LineDTO line) {
		try{
			...
			e.printStackTrace();
			HgLogger.getInstance().error("renfeng", "同步线路失败："+HgLogger.getStackTrace(e));
		}
	}
	/**
		...
		try{
			...
			if(line==null){
				...
				if(line.getLineSnapshotId()==null||!line.getLineSnapshotId().equals(lineDTO.getLineSnapshotId())){
					...
					if (nlist != null) {
						// 设置DTO中拥有的价格日历日期的ID
						Set<String> dspDtoIdSet = new HashSet<String>();
						List<DateSalePriceDTO> dspDtolist = lineDTO.getDateSalePriceList();
						if (dspDtolist != null)
							for (DateSalePriceDTO dspDto : dspDtolist)
								dspDtoIdSet.add(dspDto.getId());
						
						// 删除DTO中没有的价格日历日期
						for (DateSalePrice price : nlist) {
							if (!dspDtoIdSet.contains(price.getId()))
