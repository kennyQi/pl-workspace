...
@Entity
@Table(name=M.TABLE_PREFIX_HSL_USER+"MAILVALIDATERECORD")
public class MailValidateRecord extends BaseModel{
	 ...
	 * @param command
	 */
	public void createMailValidateRecord(SendMailCodeCommand command){
		setEmail(command.getMail());
		setCurrentTimes(0);
		setOvertime(10);
	 ...
	 * @param command
	 */
	public void createRegisterMailRecord(SendMailCodeCommand command){
		setEmail(command.getMail());
		setCurrentTimes(0);
		setOvertime(10);
		setScene(command.getScene());
		setSendDate(new Date());
		setStatus(1);
		setValue(UUIDGenerator.getUUID());
		setValidTimes(3);
		setId(UUIDGenerator.getUUID());
	}
	public void checkMailCode(String value) throws UserException{
		...
		if (getStatus() == 1) {
			...
			if (mins > this.getOvertime().longValue() 	||  this.getValidTimes() == 0) {
				setStatus(2);
				throw new UserException(UserException.VALIDCODE_OVERTIME,"短信验证码过期");
			}else if (!this.getValue().equals(value)) {
				// 判断验证码是否正确
				setCurrentTimes(this.getCurrentTimes() + 1);
				setValidTimes(this.getValidTimes() - 1);
				throw new UserException(UserException.RESULT_VALICODE_WRONG,"短信验证码错误");
			}else{
				//验证成功
				setCurrentTimes(this.getCurrentTimes() + 1);
				setValidTimes(this.getValidTimes() - 1);
