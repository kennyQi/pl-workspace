...
<body>
	...
	<header class="header">
		<h1>热门线路</h1>
		<div class="left-head">
			<a id="goBack" href="${ctx}/index" class="tc_back head-btn"> <span
								...
								<c:if test="${!empty line.lineImageList}">
									<c:forEach items="${line.lineImageList}" var="lineImage" varStatus="status">
										<c:if test="${status.index == 0 && !empty lineImage.urlMaps}">
											<img src="${imageBaseUrl}${lineImage.urlMaps['H5']}">
										</c:if>
									</c:forEach>
								</c:if>
			   ...
			   success: function(msg){
					...
					for(var i = 0; i < lists.length; i++){
						...
						if (lineObj.lineImageList != undefined && lineObj.lineImageList.length > 0
							...
							htmlStr = htmlStr + "<img src='${imageBaseUrl}" + lineObj.lineImageList[0]['urlMaps']['H5'] + "'>";
						} else {
