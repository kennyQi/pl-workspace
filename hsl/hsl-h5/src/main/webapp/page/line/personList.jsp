			        ...
			        <c:forEach items="${travelers}" var="traveler">
				        <li class="ui-border-t box" rel="${traveler.id}" choose="false" id="${traveler.id}">
				            ...
				            <h4 class="ui-nowrap person">
				                ...
				                <span class="di">
				                	${traveler.baseInfo.name}
				                	<c:if test="${traveler.baseInfo.type == 1}">（成人）</c:if>
				                	<c:if test="${traveler.baseInfo.type == 2}">（儿童）</c:if>
				                </span>
				                <label class="di"><i>手机号</i>：<i class="num mobile">${traveler.baseInfo.mobile}</i></label>
				                <label class="di"><i class="type" rel="1">身份证</i>：<i class="num cardid">${traveler.baseInfo.idNo}</i></label>
				            </h4>
				            <a href="#" class="editor pa" onclick="editTraveler()" rel="${traveler.id}">编辑</a>
				        </li>
			        </c:forEach>
			    </ul>
			</div>
