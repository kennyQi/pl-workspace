	...
	function load(orders) {
		...
		for (var i = 0; i < orders.length; i++) {
			...
			child = child.replace(/#orderNo#/gi, order.dealerOrderCode)
				...
				.replace(/#startTerminal#/gi, order.flightDTO.startTerminal?order.flightDTO.startTerminal+"航站楼":"")
				.replace(/#endTerminal#/gi, order.flightDTO.endTerminal?order.flightDTO.endTerminal+"航站楼":"")
			...
			if(payflag==(''+order.status+order.payStatus)){
				child = child.replace(/#display#/gi,'').replace(/#display_s#/gi,'none');
			}else{
				child = child.replace(/#display#/gi,'none').replace(/#display_s#/gi,'');
			}
	    ...
	    <li>
	      <div class="order_title">
	       ...
	       <div class="order_info1 order_line" onclick="detail('#orderNo#')">             
             <div class="order_info_title"><span>#startDate#</span><h3>#airCompName# #flightNo#</h3><em>#aircraftName#</em></div>
