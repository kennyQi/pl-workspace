...
//	var day = date.getDate();
	...
	for (var j = 0; j < monthAmount; j++) {
		...
		for (var i = 1; i < daysInMonth[month] + 1; i++) {
			...
			//价格日历
			//获取每个月的价格,循环加入span内
			// date_td.append("<span class='sb'>&yen;11</span>");
			for (var _index = 0; datePrices != null && _index < datePrices.length; _index++) {
				var _d = datePrices[_index].saleDate;
				var _p = datePrices[_index].adultPrice;
				var _c = datePrices[_index].childPrice;
				var _m = parseInt(_d.substring(5, _d.lastIndexOf('-')));
				_d = parseInt(_d.substring(_d.lastIndexOf('-') + 1));
				if (i == _d && _m == (month + 1)) {
					date_td.append("<span class='sb'>&yen;" + _p + "</span>");
					date_td.append("<input type='hidden' id='adultPrice' value='"+_p+"'/>");
					date_td.append("<input type='hidden' id='childPrice' value='"+_c+"'/>");
					date_td.removeClass("desibled");
				}
			}
