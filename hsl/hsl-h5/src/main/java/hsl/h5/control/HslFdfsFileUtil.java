...
public class HslFdfsFileUtil extends FdfsFileUtil{
	...
	public static FdfsFileInfo upload(InputStream fis, String extName, Map<String, String> metaMap) {
		...
		try {
			...
			HgLogger.getInstance().error("yuqz","获取fastdfs追踪服务器服节点连接失败");
			e.printStackTrace();
		}
		...
		try {
			...
			try {
				...
				HgLogger.getInstance().error("yuqz","要上传的文件不存在");
				e.printStackTrace();
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			HgLogger.getInstance().debug("yuqz","上传文件花费时间: "
					+ (System.currentTimeMillis() - startTime) + " ms");

			if (results == null) {
				HgLogger.getInstance().error("yuqz","上传文件失败, 错误码: " + storageClient.getErrorCode());
				return null;
			}
			...
			try {
				...
				HgLogger.getInstance().error("yuqz","关闭上传文件输入流失败");
				e.printStackTrace();
			}
		}
	}
