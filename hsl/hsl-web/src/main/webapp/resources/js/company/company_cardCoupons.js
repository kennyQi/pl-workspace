	...
	$(document).on("click",".donation",function(){     //点击转赠弹出窗口
		...
		$.ajax({
			...
			success: function(data) {
				...
				}else if(data.session=="needCheckMobiel"){
					
					var index=$(".donation").index($(this));
					var top1= 290+index*73+'px';
					var zzHTML='<div class="boxMessage" style="margin-top:'+top1+'"><div class="boxTitle floating_p">转赠<a href="javascript:;">'+
					'</a></div><ul class="boxMobile"><li><span>验证码：</span>'+
					'<input class="mobileNum input_p" type="text" name="" id="" maxlength="11">'+
					'<a href="javascript:;" id="sendMessage">发送短信</a></li>'+
					'<li id="errorMessage" style="padding-left:12px;">转赠需要当前登录用户验证绑定的手机号一次，请点击上方“发送短信”继续。</li>'+
					'</ul>'+
					'<a class="validateCode" href="javascript:;">确 定</a></div>'
					$(".donationBox").append(zzHTML);
					$(".donationBox").css({"display":"block"});
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(name!=="所查询的用户不存在"){
						...
						var ts='<li id="re"><i style="color:red;">所查询的用户不存在</i></li>';
						$(".boxMobile").append(ts);
					}
					$(".boxMobile").append('<li id="name">'+name+'</li>');
...
//	点击确定
	...
	$(document).on("click","#sendMessage",function(){
		...
		$.ajax({
			type:"POST",
			url:"../couponcontroller/sendValidateCode",
			dataType:"json",
			success:function(data){
				if(data.message == "验证码已发送"){
					$("#errorMessage").html("验证码已发送");
					countdown = 60;
					var val = $("#sendMessage");
					...
					$("#errorMessage").html("<label style='color: red;'>"+message+"</label>");
				}
			}
		})
	});
	...
	$(document).on("click",".validateCode",function(){
		var code = $(".mobileNum").val().trim();
		if(code == ""){
			$("#errorMessage").html("<label style='color: red;'>请输入验证码</label>");
			return false;
		}
		$.ajax({
			type:"POST",
			data:{"code":code},
			url:"../couponcontroller/validateCode",
			dataType:"json",
			success:function(data){
				...
				if(data.message =="true"){
					...
					$("#errorMessage").html("<label style='color: red;'>验证码错误</label>");
				}
			}
		})
	});
	...
	function settime(val) { 
		...
		if (countdown == 0) { 
			$("#sendMessageNo").attr("id","sendMessage");	
			val.html("重新发送"); 
			countdown = -1;
			clearInterval(timer);
			$("#sendMessage").css("background","#4bc5c3");
		} else { 
			$("#sendMessage").attr("id","sendMessageNo");
			val.html("重新发送(" + countdown + "s)");
			countdown--; 
			$("#sendMessageNo").css("background","#B5B8B8");
		} 
	}
