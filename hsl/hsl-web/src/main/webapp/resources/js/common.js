...
ply.control={
	uploadImg:function(option){
        ...
        var html='<div id="uploadImg" style="z-index:10000;"><form action="'+option.uploadUrl+'"  method="post" enctype="multipart/form-data" id="plUploadIform" target="plUploadIframe">'+
                '<input type="file" name="file"  id="fileName" style="z-index:1000;cursor:pointer;width:'+option.wid+'px;height:'+option.hei+
       ...
       option.dir.append(html);
        ...
        $("#plUploadIframe").load(function(){
            ...
            var src=xml.responseText;
            try{
            eval("src="+src);
            option.imgDir.attr("src",src.imageUrl);
            $.ajax({
        		type:"post",
        		url:"../company/infoManage/updateHeadImage",
        		dataType:"json",
        		data: {
