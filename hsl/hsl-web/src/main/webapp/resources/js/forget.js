$(function(){
	...
	if(type=="2"){
		...
		if(mobileCode!=""){
			 alertMsg("验证码有误,请重新获取",yzm1.offset().top-10,yzm1.offset().left+yzm1.closest("span").width()+10);
		}	
	}else{
		if(mobileError!=""){
		...
		if(mobileCode!=""){
			 alertMsg("验证码有误,请重新获取",yzm.offset().top-10,yzm.offset().left+yzm.closest("span").width()+10);
		}	
	}
    ...
    $("#sendYzm").click(function(){
    	...
    	var contextPath=$("#contextPath").val();
    	 if(phone.val()==""){
        	 ...
        	 $.ajax({
     			type : "POST",
     			url : contextPath+"/comRegister/sendCode2?mobile="+ phone.val() + "&scene=2",
...
function checking(id){//id==1手机 id==2邮箱
	 ...
	 if(id=="1"){
		 ...
		 if(pwd1.val()==""||pwd1.val().length<6){
		     alertMsg("请输入6-20位密码",pwd1.offset().top-10,pwd1.offset().left+pwd1.closest("span").width()+10);
		     msg=false;
		     return;
		 }
		 if(pwd2.val()==""){
	 ...
	 if(id=="2"){
    	...
    	var contextPath=$("#contextPath").val();
    	 ...
    	 if(pwd3.val()==""||pwd3.val().length<6){
    	     alertMsg("请输入6-20位密码",pwd3.offset().top-10,pwd3.offset().left+pwd3.closest("span").width()+10);
    	     msg=false;
    	     return;
    	 }
    	 if(pwd4.val()==""){
