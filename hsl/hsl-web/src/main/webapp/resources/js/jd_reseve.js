...
$(document).ready(function(){
  ...
  //景点通讯录
	...
	$(".name").blur(function() {    //验证姓名
		var  name =this.value;
		if(name.length<1){
			 $(this).siblings(".tick_ts").show().html('请输入乘客姓名');
			 $(this).addClass("ban");
		}else{
			$(this).siblings(".tick_ts").hide();
			  $(this).removeClass("ban");
		}
		});
	/**********************************************判断是企业用户还是个人***************************************************/
...
function reseve(){
	...
	//rmb='88';
	if(rmb==""){
		$(".J_DepDate").html("您还没有选择游玩日期");
		return false;
	}else{
		$('#totalPrice').val(rmb*$('#selectNum').val());
	}
	
	/*if(check=='0'||$("#goname").val()==""||$("#tel").val()==""||$("#ydname").val()==""||$("#ytel").val()==""){
		alert('页面输入有错，请检查');
		return false;
	}
*/
	if($("#goname").val()==""){
		$("#goname").next("span").html("取票人姓名不能为空");
		return false;
	}

	if($("#tel").val()==""||!pattern.test($("#tel").val())){
		$("#tel").next("span").html("取票人手机号不正确");
		return false;
	}

	if($("#ydname").val()==""){
		$("#ydname").next("span").html("预定问姓名不能为空");
		return false;
	}
	if($("#ytel").val()==""||!pattern.test($("#ytel").val())){
		$("#ytel").next("span").html("手机号不正确");
		return false;
	}
