$(function(){
	...
	var len_b=$("#carousel_b img").length;
	var wid_b=$("#carousel_b img").eq(0).width();
	var hei_b=$("#carousel_b img").eq(0).height();
	var html_b="";
	$("#carousel_b .img_list").css("width",len_b*wid_b);
	if(len_b>1){
	for(var i=0;i<len_b;i++){
		html_b+="<li>"+(i+1)+"</li>";
	}
	$("#carousel_b .icon").html(html_b);
	
	var icon_wid_b=$("#carousel_b .icon").width();
	$("#carousel_b .icon").css("left",(wid_b-icon_wid_b)/2).find("li:eq(0)").addClass("on");
	
	//按钮点击事件
	$("#carousel_b .icon li").each(function(i,e){
		$(this).click(function(){
			$("#carousel_b .icon li").removeClass("on").eq(i).addClass("on");
			$("#carousel_b .img_list").animate({left:-wid_b*i},500);
		});
	});
	
	var rel_b=$("#carousel_b .icon li.on").index();
	setInterval(function(){
		rel_b=rel_b==len_b-1?rel_b=0:rel_b+1;
		$("#carousel_b .icon li").removeClass("on").eq(rel_b).addClass("on");
		$("#carousel_b .img_list").animate({left:-wid_b*rel_b},500);
	},5000);
	}
