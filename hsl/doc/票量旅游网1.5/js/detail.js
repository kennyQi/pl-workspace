...
$(function(){
	...
	$("#starSelect span").click(function(){
		...
		var adultNum=parseInt($("#adult").html());
		var chidrenNum=parseInt($("#chidren").html());
		var name=$(this).attr("date");
		$(this).closest("#starSelect").find("label").html($(this).html());
		$(this).siblings().attr("rel","");
		$(this).attr("rel","on");
		$("#trips_money").html('<i class="h3">￥</i>'+(money*adultNum+chidrenNum*children));
	...
	$(".chooce_num .add").click(function(){
		var $num=$(this).closest(".chooce_num").find(".num")
			,minVal=parseInt($num.attr("min"))
			,totalNum=0
			;
		if(!($(this).attr("class").indexOf("on")!=-1)){
			if(minVal==parseInt($num.html())){
				$(this).closest(".chooce_num").find(".minus").removeClass("on");
			}
			$num.html(parseInt($num.html())+1);
			
			if($("#starSelect span[rel='on']").length>0){
				var money=parseInt($("#starSelect span[rel='on']").attr("money"));
				var children=parseInt($("#starSelect span[rel='on']").attr("children"));
				var adultNum=parseInt($("#adult").html());
				var chidrenNum=parseInt($("#chidren").html());
				$("#trips_money").html('<i class="h3">￥</i>'+(money*adultNum+chidrenNum*children));
	...
	//订单轮播
	var order_new_moveHeight=$(".order_new_move li").height()
		...
		;
	//初始状态设置
	$order_carous.append(order_carousHtml);
	$order_carous.find(".order_new_move").eq(1).css("top",order_new_moveHeight*order_new_moveList);
	setInterval(function(){
		var F_oldHeight=parseInt($(".order_new_move").eq(0).css("top"));
		var S_oldHeight=parseInt($(".order_new_move").eq(1).css("top"));
		...
		$(".order_new_move").eq(1).animate({top:S_oldHeight-order_new_moveHeight},300,function(){
			if(parseInt($order_carous.attr("rel"))==order_new_moveList){
				$order_carous.append($(".order_new_move").eq(0).css("top",order_new_moveHeight*(order_new_moveList-1)).detach());
				$order_carous.attr("rel",0);
			}
			$order_carous.attr("rel",parseInt($order_carous.attr("rel"))+1);
		});
	},5000);
	...
	$(window).scroll(function(){
		...
		if($(window).scrollTop()>subNavTop){
			...
			$("#subNav").css({"top":0,"position":"relative"});
			//回顶部按钮
			$(".g_totop").hide();
		}
	...
	var dayAnchorTop=$("#day_anchor").offset().top
		,dayAnchorOldTop=parseInt($("#day_anchor").css("top"))
		,dayAnchorHeight=parseInt($("#day_anchor").height());
		;
	
	var dayAnchorLeft=$("#day_anchor").offset().left;
	var dayAnchor_Left=parseInt($("#day_anchor").css("left"));
...
//			$("#day_anchor").css({"position":"absolute","top",dayAnchorOldTop});
	...
	$("#day_anchor a").each(function(i){
		$(this).click(function(){
			...
			;
			if(i==0){
				$(window).scrollTop(hei-subNavhei-40);
			}else{
				$(window).scrollTop(hei-subNavhei);
			}
			$(this).addClass("on");
...
//			}
	...
	$m_detail_carousl.each(function(i){
		var that=$(this)
			...
			,len=imglist.length
			,wid=imglist.width()
			,padR=parseInt(imglist.css("margin-right"))
			;
		...
		that.find(".right").click(function(){
			var oldLeft=parseInt(that.find(".img_list").css("left"));
			if(!that.find(".img_list").is(":animated")){
				that.find(".img_list").stop().animate({left:oldLeft-($m_detail_carousl_list*(wid+padR))},500,function(){
					if(((len*(wid+padR))+parseInt(that.find(".img_list").css("left")))<($m_detail_carousl_list*(wid+padR))){
		...
		that.find(".left").click(function(){
			...
			if(!that.find(".img_list").is(":animated")){
				that.find(".img_list").stop().animate({left:oldLeft+($m_detail_carousl_list*(wid+padR))},500,function(){
					if(parseInt(that.find(".img_list").css("left"))==0){
						that.find(".left").hide();
					}
...
$(function(){
	...
	var datetimepicker_date=$starSelect.find("span").eq(0).attr("date");
...
function dateClick(obj){
	...
	$starSelect.find("span[date='"+date+"']").trigger("click");
		$starSelect.find(".select_option").hide();
}
