...
$(document).on("click",".com1 li",function(){    //获取公司
	...
	$.ajax({
		...
		dataType:"script",
		success: function(data){
			for(var i=0;i<personnel[coms].length;i++){
				var html="<li class='l' pe="+i+">"+personnel[coms][i]+"</li>";
		        $(".com2").append(html);
				}
...
$(document).on("click",".com2 li",function(){    //获取部门
	...
	$.ajax({
		...
		dataType:"script",
		success: function(data){
			for(var i=0;i<department[coms][pers].length;i++){
				var html="<li class='l' de="+i+">"+department[coms][pers][i]+"</li>";
		        $(".com3").append(html);
				}
		}
		});
...
$(document).on("click",".com3 li",function(){   //获取个人信息
	...
	$.ajax({
		...
		success: function(data){
			 ...
			 me4=employee[coms][pers][depa][1];    //证件类型
			 me5=employee[coms][pers][depa][2];    //身份证号码
			 me6=employee[coms][pers][depa][3];    //手机号码
