...
public class HslLineIndexAdServiceImpl extends BaseSpiServiceImpl<LineIndexAdDTO, LineIndexAdQO, LineIndexAdLocalService> implements HslLineIndexAdService{
	...
	public void createLineIndexAd(CreateLineIndexAdCommand command) throws LineIndexAdException {
		...
		HgLogger.getInstance().info("chenxy", "添加线路首页广告"+JSON.toJSONString(createAdCommand));
		//先判断是否存在
		AdQO adQO = new AdQO();
		...
		if(adDTO!=null){
			try {
				...
				try {
					...
					e2.printStackTrace();
					HgLogger.getInstance().error("chenxy", "添加线路首页广告回滚广告失败"+HgLogger.getStackTrace(e2));
				}
	...
	public Pagination queryLineIndexAds(Pagination pagination) {
		...
		if(StringUtils.isBlank(qo.getShow())|| qo.getShow().equals("1")){
			adQO.setIsShow(true);
		}else{
			adQO.setIsShow(false);
		}
