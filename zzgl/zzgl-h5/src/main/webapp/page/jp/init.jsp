...
<script type="text/javascript" src="${ctx}/js/iscroll.js" ></script>
  <script type="text/javascript">
	...
	function back() {
  	...
  	function search() {
  		...
  		var startCityName =decodeURI(decodeURI($("#goCity").val()));
  		var endCityName = decodeURI($("#backCity").val());
  		location.href = "${ctx}/jp/list?openid=${openid}&orgCity=" + from + 
  			"&dstCity=" + arrive + "&startDate=" + $("#traveldate").val() + 
  ...
  <style>
	...
	/*.zx{position:fixed;bottom:60px;width:100%;}*/
	.f_nav{padding:0px 0 4px 0}
  </style>
</head>
<body style="position:relative;width:100%;">
  ...
  <div class="content"  style="overflow-x:hidden;">
    ...
    <header class="header">
      <h1>机票查询</h1>
      <div class="left-head">
        <a id="goBack" href="${ctx}/init?openid=${openid}" class="tc_back head-btn fn-hide">
      ...
      <div class="right-head">
            <a href="${ctx}/init?openid=${openid}" class="head-btn fn-hide">
                <span class="inset_shadow">
          	  ...
          	  <c:forEach var="str" items="${groups}">
          	  	...
          	  	<c:forEach var="city" items="${cities[str] }">
              		<dd cac="${city.cityAirCode }" cjp="${city.cityJianPin }"
              			cqp="${city.cityQuanPin }" ccn="${city.name}">${city.name}</dd>
              	</c:forEach>
              </c:forEach>
