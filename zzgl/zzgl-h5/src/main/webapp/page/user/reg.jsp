...
<link rel="stylesheet" href="${ctx}/css/hgsl_base.css?upt=20151202">
...
$(document).ready(function() {
	...
	$("[p_kaptchaImage='p_kaptchaImage']").click(function(){
		$("[p_kaptchaImage='p_kaptchaImage']").each(function(){
			$("[p_kaptchaImage='p_kaptchaImage']").hide().attr('src','${ctx}/user/captcha-image?'+ Math.floor(Math.random() * 100)).fadeIn();
		})
...
function doSave() {
	...
	if ($("#mobile").val().match(/1[34578][0-9]{9}/)) {
					...
					$.pop.load(false, function() {
	           			...
	           			setTimeout(function() {
	           				if (data.result == "1") {	//注册成功
	           					$.pop.tips('注册成功', 1, function() {
	           						var url = Js.decodeURL($("#redirectUrl").val());
		    		            	if (url == null || url == '') {
		    		            		url = "${ctx}/jp/init?openid=${openid}";
		    		            	}
		    		            	location.href = url;
	           					});
	    		           	} else {
    ...
    <form action="${ctx}/user/register" class="listForm" id="queryForm" method="post">
      ...
      <article class="circle_b bottom_c radius" id="payInfo">
		  ...
		  <section class="secure selectBank" style="position: relative;">
              ...
              <span class="fRight">
				   <input class="registration imgcodeInput"  name="imageCode" id="image_code" placeholder="请输入图片验证码" type="text">
                  <img id="p_kaptchaImage" src="${ctx}/user/captcha-image" class="imgcode" p_kaptchaImage="p_kaptchaImage">
              </span>
		  </section>
          ...
          <section style="position: relative;">
              ...
              <span class="fRight">
                  <input class="registration"  name="password" id="password" maxlength="20" placeholder="密码" type="password">
              </span>
          </section>
