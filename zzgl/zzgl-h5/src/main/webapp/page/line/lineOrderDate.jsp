	...
	<body>
		...
		<select id="dateMoney" style="display:none;">
			<c:forEach items="${datePrices}" var="datePrice">
			   <option value="<fmt:formatDate value="${datePrice.saleDate}" pattern="yyyy-MM-dd"/>" data-number="${datePrice.number}" for="${datePrice.childPrice}">¥${datePrice.adultPrice}</option>
			</c:forEach>
		</select>
		
		<!--表单区域-->
		<form action="${ctx}/hslH5/line/lineOrderPage" id="lineOrderDateForm">
		    <span><input name="lineId" value="${lineId}"></span>
