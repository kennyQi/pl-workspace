...
public class CouponAjaxController  extends BaseController{
	@Resource
	...
	public String selectCoupon(HttpServletRequest request,Model model){
		...
		String couponType=request.getParameter("couponType");//卡券应用类型 1机票 3线路
		...
		try {
			if (hslCouponQO.getUserId() != null&& StringUtils.isNotBlank(couponType)) {
				HgLogger.getInstance().info("zhaows", "selectCoupon-->查询卡券列表条件:" + hslCouponQO);
				hslCouponQO.setUsedKind(couponType);
	...
	public void updateCouponState(String  couponids,String OrderId,Double Price){
		try {
			...
			if(ids!=null){
				...
				HgLogger.getInstance().info("zhaows", "updateCouponState-->卡券支付时修改卡券状态成功-->参数"+consumeCouponCommand);
			}
		} catch (Exception e) {
			HgLogger.getInstance().error("zhaows", "updateCouponState-->查询卡券状态失败");
			e.printStackTrace();
		}
	}
	/**
