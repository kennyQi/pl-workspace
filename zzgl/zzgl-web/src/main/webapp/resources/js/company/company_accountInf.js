...
$(document).on("click",".checkMobis",function(){
	...
	$(document).on("click","#sendCode1",function(){
		var mobile=$("#per_mobile").attr("value"),
		 ...
		 $.ajax({
			 type: "POST",
			 url: "../comRegister/sendCode?mobile="+mobile+"&scene=3",
			 dataType:"json",
...
$(document).on("click",".countdown2",function(){
	...
	if(step2Send==true){
		...
		if(mobile!=""&&mobile.length==11&&reg.test(mobile)){
			$.ajax({
			       type: "POST",
			       url: "../comRegister/sendCode?mobile="+mobile+"&scene=4",
			       dataType:"json",
			       success: function(result) {
			    	   if(result.status=="success"){
			    		   var wait = 60;
		...
		$.ajax({
		       ...
		       success: function(result) {
		    	   if(result.flag=="success"){
					    ...
					    $("#top_mobile").html(mobile.substring(0,4)+"****"+mobile.substring(8));
	    				setTimeout(function(){
	    					$(".checkMobileBox").hide();
	    				},3000);
		    	   }else{
