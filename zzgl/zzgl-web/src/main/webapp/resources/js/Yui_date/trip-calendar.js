...
Y.TripCalendar = Y.extend(Calendar, Y.Base, {
    ...
    renderUI: function() {
        ...
        (container || $('#data')).append(this._initCalendarHTML(this.get('date')));

        this.boundingBox = Y.one('#' + this._calendarId).setStyle('float', container ? 'left' : 'left');
    ...
    _DELEGATE: {
        ...
        'click': function(e) {
            ...
            //用户添加
            document.getElementById("oder_p_val").value=target.getAttribute("rel");
        },
    ...
    _singleDateHTML: function(date) {
        ...
        for(var i = 0; i < rows; i++) {
            ...
            for(var j = 0; j <= 6; j++) {
				...
				var money = '';
				var formMoney="";
                for (var h = 0; h < dataMoney.length ; h++) {
                    if (dataMoney[h].dataM==date) {
                        money = "￥<b>"+dataMoney[h].money;
                        formMoney=dataMoney[h].money;
                        break;  
                    }
                };
				var lwmoney = days ? money:'';
				var formMoney_1 = days ? formMoney:'';
				var dbj = 'dbj'
                calday_row += 
...
},
{
    ...
    DAY_TEMPLATE: '<td onclick="changePrice($(this))"  data-date="{date}" class="{disabled}" rel="{formMoney}">' +
                        '<a href="javascript:;" id="money" class="{date_class} mouse {dbj}">' 
						+ '<span class="dataBox"><i>{day}</i><span class="money" >{money}</b></span></span>' +
                        '</a>' +                    
                    '</td>',
...
/********************************************************************************************/
   ...
   **/
   MONEYDAY : {
	   200 : ["2015-05-01","2015-05-08","2015-05-12"],
	   300 : ["2015-05-02","2015-05-06","2015-05-13"],
	   400 : ["2015-05-03","2015-05-07","2015-05-14"],
	   500 : ["2015-05-04","2015-05-09","2015-05-15"],
	   600 : ["2015-05-05","2015-05-10","2015-05-16"]
	   },
   MONEY: {
        "200"   : "￥200",
        "300" : "￥300",
