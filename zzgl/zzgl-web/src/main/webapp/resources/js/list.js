$(function(){
	...
	$.ajax({type:"post",
		...
		var	lineTopListTwo="";
		var lineBottonList="";
			$.each(data.lineTopList,function(i,item){
				if(i<2){
					lineTopListOne+="<a href="+item.adBaseInfo.url+" target='_blank'><img src="+item.adBaseInfo.imagePath+" alt= '' width='190px' height='100px'></a>";
				}else{
					lineTopListTwo+="<a href="+item.adBaseInfo.url+" target='_blank'><img src="+item.adBaseInfo.imagePath+" alt='' width='190px' height='100px'></a>";
				}
			})
			$(".img_list").append(lineTopListOne);
			$(".adTwo").append(lineTopListTwo);
			$.each(data.lineBottonList,function(i,item){
				if(i<3){
					lineBottonList+="<a href="+item.adBaseInfo.url+" target='_blank'><span><img src="+item.adBaseInfo.imagePath+" class='fl'/></span></a>";
				}else{
					lineBottonList+="<a href="+item.adBaseInfo.url+" target='_blank'><span><img src="+item.adBaseInfo.imagePath+" class='fl last'/></span></a>";
				}
			})
			$(".g_adimg").append(lineBottonList);
		}
	})	
	/*轮播*/
	///设置初始状态
...
//	
	...
	//浏览记录跟随
	if($(window).width()<1600&&$(window).width()>1180){
		$('.g_hist').css({"right":(1200-$(window).width())/2});
	}
	var g_histTop=$(".g_hist").offset().top;
	var g_histright=$(window).width()-$(".g_hist").offset().left-$(".g_hist").width();
	...
	$(window).scroll(function(){
		...
		if($(window).scrollTop()>(footTop-parseInt($(".g_hist").height()))){
			$(".g_hist").css({"top":0,"right":g_histAb_Right,"position":"absolute"});
			...
			//}
			$(".g_hist").css({"position":"fixed","right":g_histright,"top":"0"});
			//回顶部按钮
			$(".g_totop").show();
