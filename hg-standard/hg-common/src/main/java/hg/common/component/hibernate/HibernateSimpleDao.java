...
public abstract class HibernateSimpleDao {
	...
	protected Pagination findByCriteria(Criteria crit, int pageNo, int pageSize) {
		return findByCriteria(crit, pageNo, pageSize, true);
	}
	 ...
	 * @return
	 */
	protected Pagination findByCriteria(Criteria crit, int pageNo, int pageSize, boolean checkPassLastPage) {
		...
		int totalCount =0;
		//TODO tandeng 20140817 update begin
		try{
			
			Number n =  ((Number) crit.setProjection(Projections.rowCount()).uniqueResult());
			if(n==null)
		...
		// 检查页码是否超过最后一页
		if (!checkPassLastPage && pageNo > p.getPageNo()) {
			p.setList(new ArrayList());
			return p;
		}
