...
@Entity
@Table(name = M.TABLE_PREFIX + "DAY_ROUTE")
public class DayRoute extends BaseModel {
	...
	public void createDayRoute(CreateDayRouteCommand command){
		...
		if(stayLevel.length() > 0){
			stayLevel = stayLevel.substring(0,stayLevel.length()-1);
		}
	...
	public void copy(DayRoute dayRoute,Line line){
		...
		setStayLevel(dayRoute.getStayLevel());
