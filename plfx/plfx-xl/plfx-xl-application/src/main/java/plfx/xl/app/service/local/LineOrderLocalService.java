...
import plfx.xl.app.dao.LineOrderDAO;
import plfx.xl.app.dao.LineOrderPaymentDAO;
import plfx.xl.app.dao.LineOrderTravelerDAO;
...
import plfx.xl.domain.model.order.XLOrderStatus;
...
import plfx.xl.pojo.system.LineOrderConstants;
...
//				   
//				}
				HgLogger.getInstance().info("yaosanfeng", "LineOrderLocalService->modifyPaymentLineOrder->lineOrderTravelerList:" + lineOrderTravelerList.size());
				//更新游客信息
				lineOrderTravelerDAO.updateList(lineOrderTravelerList);
				//计算订单总价
...
//					&& XLOrderStatusConstant.SLFX_PAY_BARGAIN_MONEY.equals(lineOrderTraveler.getXlOrderStatus().getPayStatus() + "")){
			...
			//2当订单状态为取消订单,待退款也可以更改订单状态
			//取消订单   待退款---->取消订单  退款成功  (这一步是线下运营平台的人,手动退款后点击更改订单状态:取消订单   待退款---->取消订单  退款成功)
			if((XLOrderStatusConstant.SLFX_ORDER_CANCEL.equals(lineOrderTraveler.getXlOrderStatus().getStatus() + "") && XLOrderStatusConstant.SLFX_WAIT_REFUND.equals(lineOrderTraveler.getXlOrderStatus().getPayStatus() + ""))){
				XLOrderStatus os = new XLOrderStatus();
				os.setStatus(Integer.parseInt(XLOrderStatusConstant.SLFX_ORDER_CANCEL));//取消订单
				os.setPayStatus(Integer.parseInt(XLOrderStatusConstant.SLFX_REFUND_SUCCESS));//退款成功
				lineOrderTraveler.setXlOrderStatus(os);
			}
			LineOrderTravelerList.add(lineOrderTraveler);
		}
		lineOrderTravelerDAO.updateList(LineOrderTravelerList);
...
/*---------------------admin使用上面，shop使用下面----------------------*/	
	...
	public LineOrderDTO shopCreateLineOrder(CreateLineOrderCommand command) throws SlfxXlException{
		...
		HgLogger.getInstance().info("yuqz", "LineOrderLocalService->shopCreateLineOrder->line:" + JSON.toJSONString(line));
		...
		HgLogger.getInstance().info("yuqz", "LineOrderLocalService->shopCreateLineOrder->lineSnapshot:" + JSON.toJSONString(lineSnapshot));
		//查询订单线路团期
		DateSalePriceQO dateSalePriceQO = new DateSalePriceQO();
		...
		dateSalePriceQO.setSaleDate(travelDateStr);
		DateSalePrice dateSalePrice = dateSalePriceDAO.queryUnique(dateSalePriceQO);
		HgLogger.getInstance().info("yuqz", "LineOrderLocalService->shopCreateLineOrder->dateSalePrice:" + JSON.toJSONString(dateSalePrice));
		//查询订单适用成人价格政策
...
//		Line line = JSON.parseObject(lineSnapshot.getAllInfoLineJSON(),Line.class);
		...
		HgLogger.getInstance().info("yuqz", "LineOrderLocalService->shopCreateLineOrder->modifyDateSalePriceCommand:" + JSON.toJSONString(modifyDateSalePriceCommand));
	...
	public Boolean ShopCancleLineOrder(CancleLineOrderCommand command) throws SlfxXlException{
		HgLogger.getInstance().info("yaosanfeng", "LineOrderLocalService->ShopCancleLineOrder->CancleLineOrderCommand:" + JSON.toJSONString(command));
...
//			}
//		}
		for(LineOrderTraveler lineOrderTraveler : lineOrder.getTravelers()){
			//已取消订单的游客不能重复取消，防止团期人数重复累加
			if(!XLOrderStatusConstant.SLFX_ORDER_CANCEL.equals(lineOrderTraveler.getXlOrderStatus().getStatus() + "")){
				lineOrderTraveler.setXlOrderStatus(lineOrderTraveler.getXlOrderStatus().cancelOrderStatus());
				lineOrderTravelerList.add(lineOrderTraveler);
	...
	public LineOrderDTO modifyLineOrderTraveler(ModifyLineOrderTravelerCommand command){
		...
		lineOrderTravelerDAO.updateList(LineOrderTravelerList);
//		lineOrderDAO.update(lineOrder);
		lineOrder = lineOrderDAO.queryUnique(lineOrderQO);
		HgLogger.getInstance().info("yaosanfeng", "LineOrderLocalService->ModifyLineOrderTraveler->LineOrder:" + JSON.toJSONString(lineOrder));
		//添加日志
		DomainEvent event = new DomainEvent("slfx.xl.domain.model.line.LineOrder","ModifyLineOrderTraveler",JSON.toJSONString(command));
	    domainEventRepository.save(event);
		return EntityConvertUtils.convertEntityToDto(lineOrder, LineOrderDTO.class);	
	}
	...
	public boolean updateLineOrderStatus(UpdateLineOrderStatusCommand command) {
		...
		HgLogger.getInstance().info("yuqz", "LineOrderLocalService->travelers:" + travelers.size());
		lineOrder.setTravelers(travelers);
		HgLogger.getInstance().info("yuqz", "LineOrderLocalService->lineOrder:" + lineOrder.getTravelers().size());
