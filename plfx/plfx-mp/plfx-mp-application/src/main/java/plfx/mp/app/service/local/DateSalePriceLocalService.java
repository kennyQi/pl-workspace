...
import static plfx.mp.spi.common.MpEnumConstants.ExceptionCode.EXCEPTION_CODE_ARGS_ERROR;
import static plfx.mp.spi.common.MpEnumConstants.ExceptionCode.EXCEPTION_CODE_SALE_POLICY_NONE;
...
import plfx.mp.app.dao.DateSalePriceDAO;
...
public class DateSalePriceLocalService extends BaseServiceImpl<DateSalePrice, DateSalePriceQO, DateSalePriceDAO> {
	 ...
	 * @throws
	 */
	protected List<DateSalePrice> convertDateSalePrice(List<plfx.mp.tcclient.tc.domain.Calendar> calendar,
		...
		if (calendar != null) {
			for (plfx.mp.tcclient.tc.domain.Calendar c : calendar) {
	...
	public List<DateSalePrice> getDateSalePrice(String supplierPolicyId, String dealerId) {
		...
		if (dateSalePrices == null || dateSalePrices.size() == 0) {
			...
			List<plfx.mp.tcclient.tc.domain.Calendar> calendar = result.getCalendar();
			...
			if("0".equals(response2.getHead().getRspType())){
				...
				List<plfx.mp.tcclient.tc.domain.Calendar> calendar2 = result2.getCalendar();
