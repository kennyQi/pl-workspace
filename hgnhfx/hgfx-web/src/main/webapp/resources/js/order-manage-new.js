...
function batchSubmit(){
	$("#productIDInput").val($("#productID").val());
	$("#productIDInput1").val($("#productID").val());
	$("#sizeInput").val($(".list-content-one").size());
	$("#batchform").submit();
}
...
$(function(){
	...
	regEvent("click",".list-content-one .add",function(){
		...
		str+='<span class="list-content-item"><span>明珠卡号'+(l+1)+'</span><input type="text" name="csairCard"></span>';
		str+='<span class="list-content-item"><span>用户姓名</span><input type="text" name="csairName"></span>';
		str+='<span class="list-content-item"><span>里程</span><input type="text" name="num"></span>';
