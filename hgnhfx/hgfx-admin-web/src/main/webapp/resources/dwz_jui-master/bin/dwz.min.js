...
(function($){
	...
	$.extend(String.prototype, {
		...
		isMobile:function() {
			return (new RegExp(/^1[3578][0-9]{9}$/).test(this));
		},
		isUrl:function(){
...
(function($){
	if ($.validator) {
		...
		$.validator.addMethod("zhengzhengshu",function(value,element){
			var flag = true;
			if(value>10000000)
				flag = false;
			return this.optional(element)||/^[1-9]*[1-9][0-9]*$/.test(value)&&flag;},"请请输入1-10000000的正整数");
		$.validator.addMethod("shiqiwei",function(value,element){
			var flag = true;
			if($("#incrementVal").val().length>16||$("#incrementVal").val().length<2){
				flag = false;
			}
			return this.optional(element)||flag;},"请输入2-16位的正整数");
		
		$.validator.addMethod("reasonArea",function(value,element){
			var flag = true;
			if(value.length>200)
				flag = false;
			return this.optional(element)||flag;},"字符数超出限制，限制200");
