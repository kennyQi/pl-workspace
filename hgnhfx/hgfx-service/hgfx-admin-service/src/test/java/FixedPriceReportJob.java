...
public class FixedPriceReportJob {
	...
	public void toJob() {
		...
		for(Distributor item : distributors){
			...
			for(ProductInUse pro:list){
				...
				String yyyymm = DateUtil.formatDate1(DateUtil.parseDate2(DateUtil.forDateFirst())).substring(0, 6);
				fixedPriceIntervalSQO.setCreateDate(Integer.parseInt(yyyymm));
				FixedPriceInterval fixedPriceInterval = fixedPriceIntervalSPIService.queryFixedPriceInterval(fixedPriceIntervalSQO);
				//实际折扣
	...
	public void test(){
		FixedPriceIntervalSQO fixedPriceIntervalSQO = new FixedPriceIntervalSQO();
		fixedPriceIntervalSQO.setProductID("337cc75220a24d81b0f53c996b8e7854");
		String yyyymm = DateUtil.formatDate1(DateUtil.parseDate2(DateUtil.forDateFirst())).substring(0, 6);
		fixedPriceIntervalSQO.setCreateDate(Integer.parseInt(yyyymm));
		FixedPriceInterval fixedPriceInterval = fixedPriceIntervalSPIService.queryFixedPriceInterval(fixedPriceIntervalSQO);
		System.out.println(fixedPriceInterval);
	}
}
