...
public class ZZPLFdfsFileUtil extends FdfsFileUtil{
	...
	public static FdfsFileInfo upload(byte[] fis, String extName, Map<String, String> metaMap) {
		...
		try {
			...
			HgLogger.getInstance().error("cs","【upload】"+"获取fastdfs追踪服务器服节点连接失败"+HgLogger.getStackTrace(e));
		}
		// 存储服务器
		...
		if (metaMap != null && metaMap.size() > 0) {
			...
			String[] results = null;
			try {
				results = storageClient.upload_file(fis, extName,metaList);
				System.out.println(JSON.toJSONString(results));
			} catch (Exception e) {
				HgLogger.getInstance().info("cs", "【upload】"+"上传异常，"+HgLogger.getStackTrace(e));
			}
			HgLogger.getInstance().debug("cs","上传文件花费时间: "+ (System.currentTimeMillis() - startTime) + " ms");
			if (results == null) {
				HgLogger.getInstance().error("cs","上传文件失败, 错误码: " + storageClient.getErrorCode());
				return null;
			}
			// 获取分配到的groupName和文件名
			...
			try {
				...
				HgLogger.getInstance().info("cs", "【upload】"+"异常，"+HgLogger.getStackTrace(e));
			}
			return fdfsFileInfo;
		} 
